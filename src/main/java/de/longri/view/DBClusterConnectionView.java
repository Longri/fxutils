...
public class DBClusterConnectionView extends SelfLoading_Controller {

    private final static Logger log = LoggerFactory.getLogger(DBClusterConnectionView.class);

    @FXML
    AnchorPane fxRoot;

    public ImageView fxLogo;
    public TextArea fxServers;
    ...
    protected DBClusterConnectionView.SettingReturnListener settingReturnListener;
    protected Stage stage;

    public DBClusterConnectionView() {
        super("/views/DBClusterConnectionView.fxml");
    }
...
//        test only Server 1
        ...
        String[] ports = new String[servers.length];
        ...
        this.settingReturnListener.Return(null, fxUser.getText(), fxPassword.getText(), hosts, ports, fxDatabaseName.getText());
    }
...
//        test only Server 1
        String server[] = servers[0].split(":");
        String[] host = new String[]{server[0]};
        String[] port = new String[]{server[1]};


        if (!canConnect(messageBuilder, host, port, fxDatabaseName.getText(), fxUser.getText(), fxPassword.getText())) {
    ...
    public boolean canConnect(StringBuilder messageBuilder, String[] hosts, String[] ports, String databaseName, String user, String password) {
        return settingReturnListener.test(messageBuilder, user, password, hosts, ports, databaseName);
    }
