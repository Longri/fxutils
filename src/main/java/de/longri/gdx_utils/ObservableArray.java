...
public class ObservableArray<T> extends Array<T> implements ObservableList<T> {
    ...
    public ObservableArray(IDENTITY identity) {
        super(identity);
    }

    public ObservableArray(ObservableArray other) {
        if (other.CHANGE != null) {
            CHANGE = new ObservableArrayChange<>(other.CHANGE);
        }

        if (other.listeners.size > 0) {
            for (Object list : other.listeners) {
                this.listeners.add((ListChangeListener<? super T>) list);
            }
        }

        other.transferValuesTo(this);
    }

    protected void chkChange() {
        if (CHANGE == null) {
            CHANGE = new ObservableArrayChange<T>(this);
        }
    }
    ...
    public boolean add(T item) {
        ...
        if (val) {
            chkChange();
            CHANGE.insertAdd(item, size - 1, size - 1);
            fireChange();
        }
        return val;
    }
    ...
    public boolean removeAll(T... elements) {
        Array<T> el = new Array<>();
        el.addAll(elements);
        boolean rev = super.removeAll(el);

        if (rev) {
            chkChange();
            CHANGE.insertRemove(el);
            fireChange();
        }
        return rev;
    }
    ...
    public boolean remove(Object o) {
        boolean rev = super.remove((T) o);
        if (rev) {
            chkChange();
            CHANGE.insertRemove((T) o);
            fireChange();
        }
        return rev;
    }
     ...
     *                  specified collection
     * @param colection collection containing elements to be added to this list
     * @return {@code true} if this list changed as a result of the call
     * @throws UnsupportedOperationException if the {@code addAll} operation
     *                                       is not supported by this list
     * @throws ClassCastException            if the class of an element of the specified
     ...
     */
    @Override
    public boolean addAll(int index, Collection<? extends T> colection) {
        int start = index;
        for (T item : colection)
            super.insert(index++, item);
        chkChange();
        CHANGE.insertAdd(null, start, index - 1);
        fireChange();
        return true;
    }
    ...
    public void add(int index, T element) {
        ...
        chkChange();
        CHANGE.insertAdd(element, index, index);
        fireChange();
    }
    ...
    public T remove(int index) {
        T rev = super.remove(index);
        if (rev != null) {
            chkChange();
            CHANGE.insertRemove((T) rev);
            fireChange();
        }
        return rev;
    }
