...
import de.longri.fx.file_transfer_chooser.locations.Location;
...
final class FileChooserModel {
    ...
    private final ObjectProperty<FileTransferHandle> fileSelection = new SimpleObjectProperty<>();
    ...
    private final ObjectProperty<FileTransferHandle> pastedPath = new SimpleObjectProperty<>();

    public static FileChooserModel startingInUsersHome(PathFilter... filter) {
        return startingIn(getUsersHome(), filter);
    }

    public static FileChooserModel startingIn(FileTransferHandle startFolder, PathFilter... filter) {
    ...
    private static FileTransferHandle getUsersHome() {
    ...
    public ObjectProperty<FileTransferHandle> currentSearchPath() {
    ...
    public void setSelectedFile(FileTransferHandle file) {
        if (null == file) {
            this.fileSelection.setValue(null);
        } else {
            FileTransferHandle rootDir = fileUpdateService.searchPathProperty().get();
            this.fileSelection.setValue(rootDir);
        }
        this.invalidSelection.setValue(null == file);
    }

    public FileTransferHandle getSelectedFile() {
        return this.fileSelection.getValue();
    }

    public ReadOnlyObjectProperty<FileTransferHandle> selectedFileProperty() {
    ...
    public void updateFilesIn(FileTransferHandle directory) {
        if (null != directory) {
...
//    }
    ...
    public FileTransferHandle getPastedPath() {
        return pastedPath.get();
    }

    public ObjectProperty<FileTransferHandle> pastedPathProperty() {
        return pastedPath;
    }
}
